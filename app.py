...
# ==========================================
# 📊 2. मुख्य ऐप (MAIN APP LOGIC)
# ==========================================
def main_app():
    # साइडबार में लॉगआउट बटन
    with st.sidebar:
    ...
    st.title("📊 GeM Contract Manager & Ledger")

    # --- TABS: नया एंट्री vs पुराना इतिहास ---
    tab1, tab2 = st.tabs(["➕ New Entry / Calculator", "📜 View History"])

    # ==========================
    # TAB 1: कैलकुलेटर और एंट्री
    # ==========================
    with tab1:
        ...
        # --- रिजल्ट दिखाना ---
        st.success(f"💰 Net Profit: ₹ {net_profit:,} | 🏦 GST {gst_status}: ₹ {abs(net_gst):,}")
        ...
        # 1. Save Button
        if col_btn1.button("💾 Save Entry to History"):
            new_data = {
                "Date": [entry_date],
        ...
        pdf_data = create_pdf()
        col_btn2.download_button("📄 Download PDF", data=pdf_data, file_name=f"Report_{contract_no}.pdf", mime="application/pdf")
    ...
    with tab2:
        ...
        if os.path.exists(file_path):
            ...
            # सबसे ऊपर कुछ बड़े आंकड़े दिखाएं
            total_profit_all = df['Profit'].sum() if 'Profit' in df.columns else 0
            st.metric("🏆 Total Profit (Till Date)", f"₹ {total_profit_all:,}")
            ...
            st.download_button("📥 Download Full Excel/CSV", data=csv_data, file_name="Full_Ledger.csv", mime="text/csv")
            
        else:
            st.info("⚠️ No records found yet. Please save data first.")
